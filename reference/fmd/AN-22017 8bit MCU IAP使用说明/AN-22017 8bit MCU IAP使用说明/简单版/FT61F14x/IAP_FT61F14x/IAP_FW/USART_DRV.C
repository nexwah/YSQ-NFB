...
//	TRISA &= ~(0X1<<6/*PA6 UART_TX OUTPUT*/);  //串口默认引脚
    
	 AFP1 = 0B00010100; //UART_TX(PB6)  UART_RX(PA2) //把串口的发送和接收脚映射到程序的下载脚PA2(ISPCK),PB6(ISPDAT)
	//GPIO
	...
	URDLL=9;//115200 bps    (1M/115200=9)  
    ...
    INTCON |=(0x1<<6)|(0x1<<7);   //外设中断使能  全局中断使能
