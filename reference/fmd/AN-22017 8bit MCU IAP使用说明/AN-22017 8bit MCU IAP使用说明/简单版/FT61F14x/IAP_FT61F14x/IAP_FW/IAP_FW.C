...
void user_isr() //调用用户中断函数
{
					...
					switch(iap_databuf[0])
					{
						case CMD_E_FLASH:  //擦除闪存命令(从地址指针指定的地址开始擦除一行64个字，并在擦除整个剩余的 FLASH空间后(IAP程序以外的FLASH空间)并返回0x00                  
								...
								if(CRC_16 < IAPSIZE)   //如果不是从应用程序的起始地址开始擦除会返回错误命令
								{
								 ...
								 for(CRC_16=0x200;CRC_16<=0x1000-0x20;CRC_16+=0x20)   //对IAP程序以外的FLASH空间进行擦除,这里要注意FT61F14X的FLASH每页是32WORD
								 {
										EEADRH = CRC_H;
										EEADRL =  CRC_L;
										iap_mem_erase();
	...
	{
		asm("goto 0x204");      //跳转到应用程序中的中断函数 中去 当地址大于2k的时候  需要设置 PCLATH 设置 PC 指针高位  
	}
