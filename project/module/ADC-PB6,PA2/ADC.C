 ...
 * 输出：  无
 ...
 {
     ANSELA=0B00111111;			//控制IO的数模输入，1：对应的IO为模拟管脚，0：对应的IO为数字IO，设置AN0为模拟管脚
     ADCON0=0B00000000;
...
void main(void)
{
    ...
    while(1)
    {
		uint HTVIO_ = GET_ADC_DATA(4);
		uint an3 = GET_ADC_DATA(3);
		double HTVIO = (float)HTVIO_*2/4096;
		char msgbuf[80] = "";
		sprintf(msgbuf,"ADC:%d,%d,V:%0.2f\r\n",an3,HTVIO_,HTVIO);
